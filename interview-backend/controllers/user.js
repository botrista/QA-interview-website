...
router.put('/:user_id', errorHandler(async (req, res) => {
        
    const result = await user.put(req.params.user_id, req.body).catch(err => {
        console.log(err)
        return res.status(500).send(resFormat.return('Internal server error', 500))
    })
...
router.delete('/:user_id', errorHandler(async (req, res) => {
    ...
    const result = await user.deleteUser(req.params.user_id).catch(err => {
        console.log(err)
        return res.status(500).send(resFormat.return('Internal server error', 500))
    })
